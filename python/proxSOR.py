...
def proxSOR(
    G, d, omega, c, TOL_const, m_ineq, m_eq, MAXITER, ABSTOL, RELTOL, lambda_var
):
    niter = MAXITER
    ...
    for k in range(0, MAXITER - 1):
        ...
        for j in range(len(lambda_var)):
            ...
            if lambda_var[j] <= 0 and j >= m_eq:
                ...
                j >= m_eq and G_lambda_cmax < tmp_val + c[j] - TOL_const
            ):  ## note that the other constraints open up -> we only care about closed constraints for max(G*lambda+cmax)
        ...
        if (
            np.sqrt(sum((lambda_var - lambdaOld) ** 2))
            <= ABSTOL + RELTOL * np.sqrt(sum(lambdaOld**2))
            and G_lambda_cmax <= 0
        ):
            niter = k + 1
            break
...
def proxSORvarStep(
    G, d, omega, c, cmax, m_ineq, m_eq, MAXITER, ABSTOL, RELTOL, lambda_var
):
    niter = MAXITER
    if len(np.shape(m_ineq)) > 0:
        m_ineq = m_ineq[0]
    ...
    for k in range(0, MAXITER - 1):
        ...
        for j in range(len(lambda_var)):
            ...
            if lambda_var[j] <= 0 and j >= m_eq:
                ...
                j >= m_eq and G_lambda_cmax < tmp_val + cmax[j]
            ):  ## note that the other constraints open up -> we only care about closed constraints for max(G*lambda+cmax)
        ...
        tmp2 = (
            -1.0
            / np.dot(v.T, np.dot(G, v))
            * (np.dot(np.dot(lambdaOld.T, G), v) + np.dot(c.T, v))
        )
        tmp2 = tmp2[0, 0]
        tau_star = 1.0
        if len(tmp) > 0:
        ...
        if np.sqrt(sum((lambda_var - lambdaOld) ** 2)) <= ABSTOL + RELTOL * np.sqrt(
            sum(lambdaOld**2)
        ):  # and G_lambda_cmax <= 0:
            niter = k + 1
            break
