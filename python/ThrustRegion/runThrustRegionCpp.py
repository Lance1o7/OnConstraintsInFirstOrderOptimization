...
def runOwnSOCP(H, c, A, b, nC, x0, lambdavec, params):
    ...
    lambdavec = np.append(
        lambdavec, np.array([0.0, 0.0])
    )  # append 0.0 -> gives average number of constraints

    out = constrainedGDlib.solveSOCP(
        H_val,
        H_colidx,
        H_rowptr,
        H_params,
...
def runRandomSOCP(n):
    ...
    alpha = (
        0.4
        / (2 - 0.4)
        * (L + mu + L * (1 + np.linalg.norm(np.dot(-np.linalg.inv(H), c)) / np.sqrt(2)))
    )
    ...
    out, avg_n_constr, n_avg_ineq_constr = runOwnSOCP(
        H, c, A, b, n_C, xsol, lambdasol, params
    )
    ...
    outval = {
        "o": [tellapsed, out, niterown, avg_n_constr, n_avg_ineq_constr],
