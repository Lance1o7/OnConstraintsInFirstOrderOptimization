...
def QP(
    ...
    for k in range(1, MAXITER):
        ...
        "always activate equality constraints"
        ...
        if sum(idx) > 0:
            ...
            itertmp, lambdatmp[idx.flatten()] = proxSOR(
                Gtmp,
                np.diag(Gtmp),
                omega,
...
def SOCP(
    ...
    for k in range(1, MAXITER):
        ...
        for j in range(2, len(n_C)):
            g_ineq[n_C[1] + j - 2] = (
                -np.linalg.norm(g_tmp[idxtmp + 1 : idxtmp + n_C[j]]) + g_tmp[idxtmp]
            )
            idxtmp = idxtmp + n_C[j]
        ...
        if np.sum(idx) > 0:
            idx_1 = np.concatenate((
                idx[0 : n_C[0] + n_C[1]],
                [[False]] * (np.shape(A)[0] - n_C[0] - n_C[1]),
            ))
