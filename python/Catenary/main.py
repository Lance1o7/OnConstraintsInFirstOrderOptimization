...
z2 = np.array([[0.5, -0.8]]).T  # center of ball
r0 = 0.5  # radius of ball

np.random.seed(27111989)  # set seed
x0 = np.random.normal(size=((N - 1) * 2, 1)) * 0.05  # random initial y values
x0[0:-1:2] = np.array([np.arange(1.0 / N, 1.0, 1.0 / N)]).T  # equidistant x values
...
xsol, fsol, lambdasol, niter, prox_iter, g_ineq_tot, g_eq_tot, n_act_constr = (
    solveCatenary.solveCatenary(
        N,
        x0,
        T,
        alpha,
